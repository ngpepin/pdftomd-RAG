...
- `-l/--llm` passes `--use_llm` to Marker; set `LLM_SERVICE` in `pdftomd.conf` if you need a non-default LLM service. For OpenAI-compatible endpoints use `marker.services.openai.OpenAIService` and set `OPENAI_API_KEY`, `OPENAI_MODEL`, and `OPENAI_BASE_URL`.
